...
const SQUARE_DENSITY = Math.floor(Math.random() * 91) + 10;
...
class SQUARE {
	...
	init() {
		...
		this.size = generateDecimalBetween(10, 30);
...
class CanvasBackground {
	...
	animate() {
		...
		this.bubblesList.forEach((square) => {
			square.move();
      this.ctx.translate(square.translateX, square.translateY);
      this.ctx.beginPath();
      this.ctx.rect(0, 0, square.size, square.size);
      this.ctx.fillStyle = "rgba(" + square.color + "," + square.alpha + ")";
      this.ctx.fill();
	...
	generateSquare() {
		this.bubblesList = [];
		for (let i = 0; i < SQUARE_DENSITY; i++) {
			this.bubblesList.push(new SQUARE(this.canvas))
		}
	}
}
